...
listangaz = reader.next()
listelev = reader.next()
#The information about the slope direction of antenna in map - Azimuth
slopel= [0] * len(listangaz)
azangles= [0] * len(listangaz)
for i in range(len(listangaz)):
    azangles[i]=float(listangaz[i])
    slopel[i]=math.tan(math.radians(azangles[i]))
#Store the inclination angles as floats - Elevation
elevangles= [0] * len(listelev)
for j in range(len(listelev)):
    elevangles[j]=float(listelev[j])
"""***************************Initialization*******************************"""
...
pygame.display.set_caption("UI Ground Station Antenna")
...
while True:
    ...
    for event in pygame.event.get():
        ...
        #Using the arrow keys to change de direction angle if in Manual Mode
        elif event.type == pygame.KEYDOWN: #Azimuth
            if event.key == K_LEFT:#When left key gets pressed
                ...
                if boolmode == True:
                    if ind1 <(len(listangaz)-1): #safe limits for index
                        ind1+=1
            #You can use the up-down keys to change de Elevation angle
                ...
                if boolmode == True:
                    if ind2 <(len(listelev)-1): #safe limits for index
                        ind2+=1
    """****************************MOUSE************************************"""
    ...
    if ind1<len(listangaz) and ind2<len(listelev):
    ...
    #Chooses the end coordinates of each line to be drawn according to Azimuth
    ...
    if angledeg < 90.0:
        ...
        opy=height-endy
        opx=width-endx
        #Calculate the (x,y) coordinates in the line according to Elevation
    ...
    #When angle between 90 and 179 degrees of Azimuth
    elif angledeg < 180.0:
        if angledeg == 90:
            endy=height/2
            endx=0
        ...
        coordx=(width/2)-(math.sin(math.radians(angledeg)))*((scalex)*((90-elevang)/90))
        coordy=(height/2)-(math.cos(math.radians(angledeg)))*((scaley)*((90-elevang)/90))
    #When angle between 180 and 269 degrees of Azimuth
    elif angledeg < 270.0:
        if angledeg == 180:
            endy=height
            endx=width/2
            ...
            coordx=(width/2)-(math.sin(math.radians(angledeg)))*((scalex)*((90-elevang)/90))
            coordy=(height/2)-(math.cos(math.radians(angledeg)))*((scaley)*((90-elevang)/90))
    #When angle between 270 and 360 degrees of Azimuth
    elif angledeg <= 360.0:
        if angledeg == 270:
            endy=height/2
            endx=width
    ...
    if boolzoom == True:
        layer = pygame.image.load("mapin.png") #Map 1 chosen
        scalex=(width/2)*9
        scaley=(height/2)*9
    else:
    ...
    circul=pygame.image.load("bluecircle.png") #Blue circle target
    switch1=pygame.image.load("switch1.png") #Load all switches images
    switch2=pygame.image.load("switch2.png")
    switch3=pygame.image.load("switch3.png")
    switch4=pygame.image.load("switch4.png")
    """*************************UPDATE SCREEN*******************************"""
    pygame.display.update()
    #Update text in the indication section
    textang1=myfont.render("Azimuth: "+str(angledeg)+" degrees",0,GREEN)
    ...
    """*****************************CURSOR*********************************"""
    #Draw cursor in map
    if boolcursor==True: #cursor follows mouse
        screen.blit(cursor,(arrowx,arrowy))
    else: #cursor arrow stops in saved location
